...
# ─────────────────────────────────────────────────────────────
# CutTheCord — Self-Hosted Setup Script (Arch Linux Edition)
# ─────────────────────────────────────────────────────────────
...
if command -v docker &>/dev/null; then
    ...
    # Arch specific: Ensure the service is actually running
    if ! systemctl is-active --quiet docker; then
        warn "Docker is installed but not running."
        info "Try running: sudo systemctl enable --now docker"
    fi
else
    missing+=("docker")
fi
...
if [[ ${#missing[@]} -gt 0 ]]; then
    ...
    echo ""
    info "Install them using pacman:"
    printf "  ${BOLD}sudo pacman -S ${missing[*]}${RESET}\n"
    exit 1
fi
echo ""
...
# ARCH FIX: Use `ip route` instead of `hostname -I` for reliable LAN IP detection
DETECTED_IP=$(ip route get 1.1.1.1 2>/dev/null | grep -oP 'src \K\S+') || DETECTED_IP=""
...
# ARCH FIX: Regex validation to prevent bad config files
if [[ ! $HOST_IP =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    die "Invalid IP format. Please use XXX.XXX.XXX.XXX"
fi
